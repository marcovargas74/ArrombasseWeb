...
//initialize array
$limitquery=50000;  //maximum number of attack used to calculated attack details
$listip=array();
$countrycode=array();
$nbrcountry=array();
$listcountry=array();
...
$requete = mysql_query($sql, $connexion) or die("MySQL query error1");
...
$requetestats = mysql_query($sqlstats, $connexion) or die("MySQL query error2");
...
if($nbrresult>=1)
	...
	$requete = mysql_query($sql, $connexion) or die("MySQL query error3");
    while ($ligne = mysql_fetch_row($requete))                                                                              
        {
    ...
    while ($ligne = mysql_fetch_row($requetestats))                                                                              
        ...
        if($ligne[1]!='')
            ...
            @$nbrvisits[$ligne[1]]++; 
            ${'crawler'.$ligne[1]}[$crawlername[$ligne[0]]] =$crawlername[$ligne[0]];
    ...
    $requeteexistingip = mysql_query($sqlexistingip, $connexion) or die("MySQL query error4");
    ...
    foreach ($listip as $ip)
        ...
        if(!isset($countrycode[$ip]))
            ...
            $requetestats = mysql_query($sqlstats, $connexion) or die("MySQL query error5");
          ...
          //enter the ip data in the crawlt_ip_used table
            ...
            $requeteinsertip = mysql_query($sqlinsertip, $connexion) or die("MySQL query error6");                    
            } 
         else
            {
            $code=$countrycode[$ip];
    ...
    $requete = mysql_query($sql, $connexion) or die("MySQL query error7");
    ...
    */
    ...
    if($nbrresult < $limitquery)
      ...
      $sqlstats = "SELECT COUNT(DISTINCT id_visit),COUNT(DISTINCT crawlt_ip_used) FROM crawlt_visits, crawlt_pages
      WHERE  crawlt_visits.crawlt_pages_id_page=crawlt_pages.id_page 
      $testcrawler
      AND $datetolookfor       
      AND crawlt_visits.crawlt_site_id_site='".sql_quote($site)."'
      ...
      $requetestats = mysql_query($sqlstats, $connexion) or die("MySQL query error8");
      $ligne = mysql_fetch_row($requetestats);
      
      $nbrattack=$ligne[0];
      $nbrip2=$ligne[1];
    
        //estimation of the number of attacker using the same proportion       
      $nbrattacker = $nbrip2*( count($nbrvisits2)/$nbrip);         
      }
      
      
	//display---------------------------------------------------------------------------------------------------------
    echo"<div class=\"content\">\n";
    echo crawltbackforward('hacking3',$period,$daytodaylocal,$monthtodaylocal,$yeartodaylocal,$daybeginlocal,$monthbeginlocal,$yearbeginlocal,$dayendweek,$monthendweek,$yearendweek,$crawler,$navig,$site,$graphpos);  
    echo"</div>\n";
          ...
          //Initialize array & variables
            ...
            foreach($tableauip as $ip)
                ...
                foreach (${'crawler'.$ip} as $crawler)
                    {
                    $listcrawler[$crawler]=htmlentities($crawler);
                    }
                $nbrgoodsite=0;
